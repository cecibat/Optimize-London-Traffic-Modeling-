...
cons5(i,j)    only existing link can be used
cons6(i,j)    set the level of congestion
cons7(i,j)    congestion equal to zero if no road
cons8(i,j)    maximum congestion
obj           obj funct;

cons1(i,j)                     .. x(i,j)        =E= sum(k,w(k,i,j));
cons2(i,j)$(ord(i) ne ord(j))  .. d(i,j)        =E= sum(k, w(i,k,j) - w(i,j,k));
cons3(i)                       .. sum(j,d(i,j)) =E= sum(j,w(i,i,j));
cons4(i,j)$(u(i,j) > 0)        .. x(i,j)        =L= u(i,j)*0.99;
cons5(i,j)$(u(i,j) eq 0)       .. x(i,j)        =L= 0;
cons6(i,j)$(u(i,j) > 0)        .. cong(i,j)     =E= x(i,j)/u(i,j);
cons7(i,j)$(u(i,j) eq 0)       .. cong(i,j)     =E= 0;
cons8(i,j)                     .. maxCong       =G= cong(i,j);
obj                            .. z             =E= sum((i,j)$(u(i,j)>0), x(i,j)*( t(i,j)+ (a(i,j)*x(i,j)) / (u(i,j)-x(i,j)) ) );


model project /all/;
solve project using nlp minimize maxCong;

display x.l , z.l, w.l, maxCong.l, cong.l;
